...
def convert_to_xml(result_path: str, gold_path: str, max_length_diff=None):
    ...
    output_dir = os.path.join("./result_xmls/", output_dir_name)
    os.makedirs(output_dir, exist_ok=True)

    pred_xml_path = os.path.join(output_dir, "pred.xml")
    ref_xml_path = os.path.join(output_dir, "ref.xml")

    pred_triplets = [
        l.strip() for l in open(result_path, "r", encoding="utf-8").readlines()
    ]
    gold_triplets = [
        l.strip() for l in open(gold_path, "r", encoding="utf-8").readlines()
    ]
    ...
    for idx, triplets in enumerate(pred_triplets):
        try:
            ...
            if idx < len(gold_triplets):
                collected_gold_triplets.append(ast.literal_eval(gold_triplets[idx]))
        except Exception:
            if idx < len(gold_triplets):
                collected_gold_triplets.append([])
            pass

    min_len = min(len(collected_pred_triplets), len(collected_gold_triplets))
    collected_pred_triplets = collected_pred_triplets[:min_len]
    collected_gold_triplets = collected_gold_triplets[:min_len]
    ...
    pred_tree.write(pred_xml_path, encoding="utf-8")

    gold_tree = ET.ElementTree(gold_root_node)
    ET.indent(gold_tree, space="\t", level=0)
    gold_tree.write(ref_xml_path, encoding="utf-8")
...
def main_row_by_row_comparison(reffile, candfile):
    """
    一个高效的、严格按行进行两种比较的评估函数。
    """
    print("--- Running in optimized row-by-row comparison mode ---")

    # 1. 加载和预处理数据
    reflist_orig, newreflist = getRefs(reffile)
    candlist_orig, newcandlist = getCands(candfile)

    min_len = min(len(newreflist), len(newcandlist))
    reflist_orig, newreflist = reflist_orig[:min_len], newreflist[:min_len]
    candlist_orig, newcandlist = candlist_orig[:min_len], newcandlist[:min_len]

    # 初始化两种比较的总计数器
    # A. 精确匹配计数器
    total_exact_tp, total_exact_fp, total_exact_fn = 0, 0, 0
    # B. 部分匹配(nervaluate)结果收集器
    all_partial_scores = []

    # 2. 逐行循环比较
    print(f"Processing {min_len} entries...")
    for i in range(min_len):

        # --- A. 执行快速的精确匹配 ---
        ref_set = set(t.lower() for t in reflist_orig[i])
        cand_set = set(t.lower() for t in candlist_orig[i])

        total_exact_tp += len(ref_set.intersection(cand_set))
        total_exact_fp += len(cand_set - ref_set)
        total_exact_fn += len(ref_set - cand_set)

        # --- B. 部分匹配 ---
        # 如果当前行没有标准答案或候选答案，则跳过
        if not newreflist[i] or not newcandlist[i]:
            continue

        # 贪心匹配：为每个候选三元组找到得分最高的标准三元组
        # 创建一个列表来标记标准三元组是否已被匹配，避免重复
        matched_ref_indices = [False] * len(newreflist[i])

        for cand_triple in newcandlist[i]:
            best_score = -1
            best_ref_idx = -1
            best_results = None

            for ref_idx, ref_triple in enumerate(newreflist[i]):
                # 如果这个标准答案已经被用过了，就跳过
                if matched_ref_indices[ref_idx]:
                    continue

                # 计算部分匹配分数
                results, _ = evaluaterefcand(ref_triple, cand_triple)
                current_f1 = results["strict"]["f1"]
            ...
            # 如果找到了一个匹配 (即使分数为0)，就记录结果并标记
            if best_ref_idx != -1:
                all_partial_scores.append(best_results)
                matched_ref_indices[best_ref_idx] = True

    # 3. 计算并打印最终结果
    # A. 精确匹配结果
    precision = (
        total_exact_tp / (total_exact_tp + total_exact_fp)
        if (total_exact_tp + total_exact_fp) > 0
        else 0
    )
    recall = (
        total_exact_tp / (total_exact_tp + total_exact_fn)
        if (total_exact_tp + total_exact_fn) > 0
        else 0
    )
    f1 = (
    ...
    print("\n--- [FAST] Exact Triple Match Results ---")
    print(f"Precision: {precision:.4f}, Recall: {recall:.4f}, F1 Score: {f1:.4f}")
    print("-----------------------------------------")

    # B. 部分匹配结果 (汇总)
    if all_partial_scores:
        # 汇总 Strict 指标 (精确：词、边界、类型全对)
        strict_p_avg = statistics.mean(
            [s["strict"]["precision"] for s in all_partial_scores]
        )
        strict_r_avg = statistics.mean(
            [s["strict"]["recall"] for s in all_partial_scores]
        )
        strict_f1_avg = statistics.mean([s["strict"]["f1"] for s in all_partial_scores])

        # 汇总 Partial 指标 (部分：词、边界对即可)
        partial_p_avg = statistics.mean(
            [s["partial"]["precision"] for s in all_partial_scores]
        )
        partial_r_avg = statistics.mean(
            [s["partial"]["recall"] for s in all_partial_scores]
        )
        partial_f1_avg = statistics.mean(
            [s["partial"]["f1"] for s in all_partial_scores]
        )

        print("\n--- [SLOWER] Partial Match Results (Detailed Averaged) ---")
        print(
            f"Strict  (全对): Precision: {strict_p_avg:.4f}, Recall: {strict_r_avg:.4f}, F1: {strict_f1_avg:.4f}"
        )
        print(
            f"Partial (词对): Precision: {partial_p_avg:.4f}, Recall: {partial_r_avg:.4f}, F1: {partial_f1_avg:.4f}"
        )
        print("--------------------------------------------------")
    else:
        print("\nNo partial matches found or calculated.")
...
if __name__ == "__main__":
    ...
    # 1. 配置区域：直接指定要评估的文件路径
    # ==================================================================
    # PRED_FILE_PATH = "ablation/description/webnlg/Qwen3-30B-A3B-Instruct-2507_few_shot_True/eval_prediction4.txt"
    PRED_FILE_PATH = "output/stanfordOIE/webnlg_results.txt"

    # --- 指定标准答案文件的路径 ---
    GOLD_FILE_PATH = "datasets/webnlg/webnlg_gold_for_eval.txt"
    ...
    # 2. 执行评估
    # ==================================================================
    print("--- 开始评估 (模式: 手动指定文件) ---")
    print(f"  预测文件 (Prediction File): {PRED_FILE_PATH}")
    print(f"  标准文件 (Gold Standard File): {GOLD_FILE_PATH}")
    print("----------------------------------------\n")

    # 检查文件是否存在
    if not os.path.exists(PRED_FILE_PATH):
        print(f"错误：找不到预测文件 '{PRED_FILE_PATH}'。请检查路径是否正确。")
        exit()
    if not os.path.exists(GOLD_FILE_PATH):
        print(f"错误：找不到标准答案文件 '{GOLD_FILE_PATH}'。请检查路径是否正确。")
        exit()

    # 转换文件格式
    result_xml_path, gold_xml_path = convert_to_xml(
        PRED_FILE_PATH, GOLD_FILE_PATH, MAX_LENGTH_DIFF
    )
    print(f"\n已生成临时XML文件用于评估：")
    print(f"  预测XML: {result_xml_path}")
    print(f"  标准XML: {gold_xml_path}\n")

    # 运行核心评估逻辑
    main_row_by_row_comparison(gold_xml_path, result_xml_path)
